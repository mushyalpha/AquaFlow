 ...
 *
 * Uses libgpiod (the officially supported GPIO library for Linux)
...
public:
  enum class DriveMode { RELAY_ACTIVE_LOW, TRANSISTOR_ACTIVE_HIGH };
  ...
  PumpController(unsigned int chipNo, unsigned int pumpPin,
                 DriveMode mode = DriveMode::TRANSISTOR_ACTIVE_HIGH);
...
private:
  ...
  std::optional<gpiod::line> pumpLine_;
  std::optional<gpiod::line> ledLine_;
};
